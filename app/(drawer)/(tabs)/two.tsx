...
export default function Gabscreen() {
  ...
  const { data, error, isLoading, refetch } =useGetviralGabQuery(queryCoordinates, {refetchOnMountOrArgChange:true});
  ...
  return (
    <>
      <FlatList data={data} renderItem={({ item }) => <HotGab gab={item} />} />
      <Footergab />
    </>
  );
}
